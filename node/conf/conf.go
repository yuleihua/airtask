...
var DefaultConfig = &Config{
	...
	HTTPModules: []string{"admin", "task"},
	HTTPPort:    DefaultHTTPPort,
	WSOrigins:   []string{"*"},
	WSModules:   []string{"admin", "task"},
	WSPort:      DefaultWSPort,
}

func NewConfig(name, version, dataDir, host string, level int) *Config {
	return NewConfigEx(name, version, "", dataDir, host, "127.0.0.1", level, DefaultHTTPPort, DefaultWSPort)
}

func NewConfigEx(name, version, nodeId, dataDir, host, wsHost string, level, port, wsPort int) *Config {
	httpHost := host
	if host == "" {
		httpHost = "127.0.0.1"
	}
